...
public final class JFXVideo extends AbstractPlayableVideo {
    ...
    // The following values will not be valid until the movie is ready
    // (pre-rolled)
    ...
    /*
     * All JavaFX scene commands must be done on the event dispatch thread (not
     * the Swing thread). We are going to do a bit of an odd thing hear to
     * account for some ideosyncratic behaviour of the old Quicktime interface
     * that we are replacing. While the Quicktime interface was always
     * ultimately asynchronous, it had certain workarounds in place to make it
     * behave as if it were synchronous - namely the pre-roll routines could be
     * treated as if synchronous. This behaviour is not present in nearly any
     * first class, modern media players (see for instance the Mac OSX/iOS
     * AVFoundation routines). Unfortunately there are many apparent
     * dependencies within Korsakow that rely on the old synchonous behaviour
     * (for instance, MediaInfoFactory expected the QTVideo constructor to
     * return immediately with the movie already pre-rolled). So we have a
     * problem - either we reconstruct Korsakow to a more non-linear, event
     * driven approach when relying on media loading - which could take quite a
     * bit of work - or we attempt to make the pre-roll of the media
     * synchronous. In favor of symplifying the outer code that relies on these
     * classes, the following is an attempt to cause the pre-roll to load
     * syncronously. The side effect is that this could cause the entire
     * application to hault if there are lengthy operations queued on the JavaFX
     * thread, or if the video is slow to pre-load.
     */
    private synchronized void initializeScene() throws MediaRuntimeException {
        try {
            ...
            // block the current thread until the JafaFX Application thread
            // returns.
    ...
    private void createJFXScene() throws MediaRuntimeException {
        try {
            ...
            moviePlayer.setOnReady(new Runnable() {
                ...
                public synchronized void run() {
                    ...
                    moviePanel.setPreferredSize(new Dimension(moviePlayer.getMedia().getWidth(), 
                                                moviePlayer.getMedia().getHeight()));
            ...
            moviePlayer.currentTimeProperty().addListener(
            ...
            moviePlayer.volumeProperty().addListener(
                    new InvalidationListener() {
                        @Override
                        public void invalidated(Observable o) {
                            currentVolume.set(Double.doubleToLongBits(moviePlayer.getVolume()));
            ...
            moviePlayer.totalDurationProperty().addListener(
                    new InvalidationListener() {
                        @Override
                        public void invalidated(Observable o) {
                            duration.set((long) moviePlayer.getTotalDuration().toMillis());
        ...
        moviePlayer.statusProperty().addListener(new InvalidationListener() {
            @Override
            public void invalidated(Observable o) {
    ...
    public void dispose() {
        ...
        if (movieView != null) {
            Platform.runLater(new Runnable() {
                ...
                public void run() {
                    try {
                        // This will ensure that we have cleaned up any media
                        // resources
                        if (moviePlayer != null) {
                            moviePlayer.stop();
                            moviePlayer.dispose();
